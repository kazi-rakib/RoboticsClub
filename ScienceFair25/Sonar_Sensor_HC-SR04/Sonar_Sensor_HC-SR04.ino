...
int potValue = 15;    // variable to read the value from the analog pin
...
void loop() {
  ...
  potValue = map(potValue, 0, 1023, 15, 400);     // scale it for use with the servo (value between 0 and 180)


  if (distance < potValue){
    digitalWrite(loadPin, LOW); // turns ON the relay i.e., connects COM to NO
    tone(buzzerPin, 4000);
    
  } else {
    digitalWrite(loadPin, HIGH); // turns OFF the relay i.e., connects COM to NC
    noTone(buzzerPin);
  }
